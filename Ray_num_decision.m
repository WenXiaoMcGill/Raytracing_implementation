...
start_number = 250; % start ray number
interval = 20; % interval between points
step = 5; % interval numbers
T_repeat = 20; % repeating times in each loop
...
% N = 200*200;            % Set up the number of the rays ommited must be power of 2
...
for Num = 1:1:step
    Ecomp = 0;
    N = (start_number+(Num-1)*interval)^2;
    for nn = 1:1:T_repeat+1
        %-----------------------------------------------------------------%
        ...
        dev(1,Num) = dev(1,Num) + sum(abs(E-Ecomp));
        Ecomp = E;
        
%         IR_avrg = IR_avrg+IR;
    end
    
    dev(1,Num) = dev(1,Num)/T_repeat/N;
    dev(2,Num) = sqrt(N);
%     dev = log10(dev/size(IR,2)/(1e-12));
    disp(dev(1,Num))
    
%     IR_avrg = IR_avrg/(T_repeat+1);
end
